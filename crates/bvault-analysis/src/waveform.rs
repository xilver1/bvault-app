...
use bvault_core::{
    Waveform, WaveformColorEntry, WaveformColorPreview, WaveformColorPreviewColumn, WaveformColumn,
...
impl WaveformGenerator {
    ...
    fn generate_color_preview(&self, samples: &[f32]) -> WaveformColorPreview {
        ...
        for i in 0..1200 {
            ...
            let bass_energy =
                band_energy(&fft_buffer, bass_start.max(1)..=bass_end.min(fft_size / 2));
            ...
            let luminance =
                ((bass_energy + mid_energy + high_energy) * boost).clamp(0.0, 127.0) as u8;
    ...
    fn generate_detail(&self, samples: &[f32], duration_secs: f64) -> WaveformDetail {
        ...
        let r_band = pct95(raw.iter().map(|e| e.0.max(e.1).max(e.2)).collect());
        ...
        for (bass_energy, mid_energy, high_energy, amplitude) in raw {
            ...
            entries.push(WaveformColorEntry {
                red,
                green,
                blue,
                height,
