//! rekordbox-export: renders a USB export's "brain" — `export.pdb`, the
...
//! - **Core is untouched.** This crate only *orchestrates* rekordbox-core's
...
pub enum Error {
    ...
    #[error("store: {0}")]
    Store(#[from] rekordbox_store::Error),

    #[error("core: {0}")]
    Core(#[from] rekordbox_core::Error),
