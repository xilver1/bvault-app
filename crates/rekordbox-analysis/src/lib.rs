//! rekordbox-analysis: pure audio-analysis front-end.
//!
...
use rekordbox_core::{BeatGrid, TrackAnalysis};
...
pub fn analyze_path(path: &Path, opts: &AnalyzeOptions) -> Result<TrackAnalysis> {
    ...
    let file_hash =
        rekordbox_core::compute_file_hash(path).map_err(|e| Error::Hash(e.to_string()))?;
